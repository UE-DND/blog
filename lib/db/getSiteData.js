 ...
 * 获取首页文章
 */
async function getHomePost(post) {
  if (!post) {
    return null
  }

  post.blockMap = await getPage(post.id, 'data-home')
  return post
}
...
function handleDataBeforeReturn(db) {
  ...
  if (db?.notice) {
    ...
    delete db.notice?.blockMap?.collection_view
  }
  
  // 处理首页文章数据
  if (db?.homePost) {
    db.homePost = cleanBlock(db?.homePost)
    delete db.homePost?.blockMap?.collection_view
  }
