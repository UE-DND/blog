...
function AsideLeft(props) {
  ...
  return (
    <div
      className={`sideLeft relative ${isCollapsed ? 'w-0' : 'w-80'} bg-white dark:bg-hexo-black-gray min-h-screen hidden lg:block z-20 animated`}
      ...
      <div className={`h-full ${isCollapsed ? 'hidden' : 'p-8'}`}>
        ...
        {isPostPage && post?.toc && post.toc.length > 0 && (
          <section className='sticky top-0 pt-12 flex flex-col max-h-screen'>
            ...
            <div className='catalog-wrapper bg-white dark:bg-hexo-black-gray overflow-y-auto' style={{ maxHeight: 'calc(100vh - 300px)' }}>
...
export function SidebarButtons() {
  ...
  const toggleOpen = () => {
    setIsCollapse(!isCollapsed)
    // 触发全局事件，使主组件也能响应
    const event = new CustomEvent('toggleSidebar', { detail: !isCollapsed })
